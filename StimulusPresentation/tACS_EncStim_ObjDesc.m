% function [enc_out,msg]=tACS_EncStim_ObjDesc(thePath)
...
% for a given duration. subjects will be making a shallow encoding
% decisions. In this case it is smaller or bigger than a shoe box.
...
% LastUpdate:   September 13th, 2016
%------------------------------------------------------------------------%
...
if exist(fileName,'file')
    ...
    tacs_er = tACS_ER_makelist_ObjStims(thePath);
end
...
PresParams.ITI_Range            = [0.5 1.5]; % variable ITI in secs
...
if PresParams.tACSstim
    ...
    PresParams.StimTemplate         = 'AG_tACS_ER_6Hz';
...
try
    ...
    InstStr = ['Instructions\n\n' ...
        ...
        'If no questions, \n'...
        'Let the experimenter know you are ready to start the experiment.'];
    
    PauseStr = ['Rest Pause. \n\n '...
        'To continue press the ''' resumeKey ''' key.' ];
    
    DrawFormattedText(window,InstStr, 'wrapat', 'center', 255, 75, [],[],[],[],[xCenter*0.2,0,screenXpixels*0.6,screenYpixels]);
    Screen('Flip',window);
    ...
    for tt = 1:nTrials
        ...
        % Draw Post-Stim Blank after completing previous cycle
        [flip.VBLTimestamp, flip.StimulusOnsetTime, flip.FlipTimestamp, flip.Missed, flip.Beampos,] ...
        ...
        if mod(tt,PresParams.PauseEvNtrials)==0
            DrawFormattedText(window,PauseStr, 'center', 'center', 255, 75, [],[],[],[],[xCenter*0.2,0,screenXpixels*0.6,screenYpixels]);
            Screen('Flip',window);
            WaitTillResumeKey(resumeKey,activeKeyboardID)
        end
    ...
    % save
    fileName = 'tacs_er_objstim.enc.mat';
    cnt = 0;
    while 1
        ...
        if ~exist(savePath,'file')
            ...
            fileName = strcat('tacs_er_objstim.enc','-',num2str(cnt),'.mat');
