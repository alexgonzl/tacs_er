function [tacs_er] = tACS_ER_makelist_Expt3a(thePath)
...
% 3) The decision will be indoor vs outdoor images
%
% Important variables
% subjNum       -> subject  ID
...
%
% EncStimType       -> VECTOR trial of indicating type (1  indoor, 2 outdoor)
% EncStimTypeIDs    -> {In,Out}
% EncCondCodeIDs    -> {'In000','In036','In072','In108'...,'Out000',...}; 
...
%
% RetCondIDs        -> {'OldIn','OldOut','NewIn','NewOut'};
...
load([thePath.stim,'/scene_categories/selInOutImgsV2.mat']);
...
%
% EncStimType -> 1 for inside, 2 for outside
% EncStimNames -> name of the stimuli as it apperas on the database

% order of inside and outside per block
EncStimTypeIDs  = {'In','Out'};
...
% assign in/out trials at encoding
...
EncCondCodeIDs(stim1ObjConds) = strcat('in',cellfun(@num2str,num2cell(EncPhases'),'UniformOutput',false));
EncCondCodeIDs(stim2ObjConds) = strcat('out',cellfun(@num2str,num2cell(EncPhases'),'UniformOutput',false));
